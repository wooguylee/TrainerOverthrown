...
## 2026-08-25 반복 씬 검색 성능 회귀 제거

- 사용자 실행에서 화면이 주기적으로 끊겼고, 같은 세션 로그에는 원형 메뉴 번역 적용
  기록이 한 번도 없었다. 직전 구현은 메뉴가 없을 때도 `FindObjectOfType`를 초당 5회
  호출했으므로 번역에 기여하지 않으면서 메인 스레드 씬 검색을 반복했다.
- 게임이 이미 유지하는 `RadialMenu.currentActive` 정적 집합으로 열린 메뉴에 직접
  접근하도록 바꿔 원형 메뉴 한글화 경로의 `FindObjectOfType`를 완전히 제거한다.
  활성 메뉴가 없으면 정적 집합의 null/개수 확인만 하고 종료한다. 해당 interop
  `HashSet` 형식을 컴파일하기 위해 게임에 이미 설치된 `Il2CppSystem.Core.dll`을
  `Private=false` 참조로만 추가하며 별도 런타임 파일은 복사하지 않는다.
- 무적 모드의 `FindObjectsOfType<PlayerHealth>()`도 매 프레임 호출하지 않는다. 로컬
  플레이어의 `Damageable`을 한 번 찾은 뒤 캐시하고, 대상이 없거나 파괴된 동안에만
  최대 1초에 한 번 재탐색한다. 무적 모드가 꺼져 있으면 기존처럼 즉시 종료한다.
...
## 2026-08-25 문자열 테이블 한글의 TMP 폰트 보완

- 연구 화면의 `채집` 상세 팝업에서 설명은 정상 표시되지만 제목과 자원 항목 한글이
  네모로 표시되는 현상을 확인했다. 번역 누락이 아니라 해당 팝업이 사용하는 TMP 폰트에
  한국어 glyph fallback이 연결되지 않은 증상이다.
- 문자열 테이블 일회성 교체 뒤에는 `TMP_Text.set_text` 입력이 이미 한글이므로, 기존
  패치가 추가 번역이 없다고 판단해 fallback 연결도 건너뛰고 있었다. 최종 입력에 한글이
  포함됐으면 번역 여부와 무관하게 해당 폰트를 보완하도록 수정했다.
- 성능 영향을 제한하기 위해 한글 입력에서만 폰트를 확인하고, native font pointer별로
  성공한 연결을 캐시한다. 같은 폰트에는 fallback 목록 조회와 추가를 반복하지 않으며
  전역 UI 탐색, 주기 감시, 프레임별 component 순회는 추가하지 않았다.
- Helper와 Helper.Core를 Release로 빌드했으며 경고 0, 오류 0이다. staged payload와
  기존 앱 payload를 갱신한 뒤 게임이 종료된 상태에서 안전 제거·재설치를 완료했다.
  제거·설치 exit code는 모두 0이며 설치 manifest 233개 파일의 누락, 길이 불일치,
  SHA-256 불일치는 모두 0개다. 사용자 지침에 따라 자동 테스트, 게임 실행 테스트,
  ZIP 패키징은 실행하지 않았다.

## 2026-08-25 확장 테스트 트레이너

- Helper pipe가 실제로 연결되지만 `BNetworkManager.OfflineMode`가 false여서 앱이
  `Uncertain`으로 기능 전체를 잠그던 상태를 확인했다. 사용자는 이미 로컬 월드에 있었으므로
  세션 플래그를 기능 승인 조건으로 쓰지 않고 진단 정보로만 남기는 테스트 모드로 전환했다.
- 현재 빌드 세 파일의 해시 검사는 그대로 유지한다. 각 명령은 세션 판정 대신 로컬
  플레이어, 인벤토리, 난이도 관리자, 왕국 저장소 등 필요한 실제 객체의 준비 상태를
  검사하며 미로드 상태는 `FEATURE_UNAVAILABLE`로 반환한다.
- 기능을 `플레이어`, `이동·시간`, `인벤토리`, `왕국 자원`, `진단` 탭으로 나눴다. 체력
  회복·무적·기력 소모 없음, 이동 속도, 시간 일시정지/가속, 36종 자원의 조회·설정·증감,
  원시 세션/객체/capability 진단을 제공한다.
- 인벤토리는 `PlayerInventory`의 게임 메서드와 자원 기본 아이템을 사용하고, 왕국 자원은
  `GlobalResourceStorage.Deposit/Withdraw`를 사용한다. 두 자원 변경은 저장 상태이므로
  자동 복원하지 않으며 실게임 자동 확인에서는 조회만 수행한다.
- IPC 계약과 값 범위/자원 ID 검증 테스트를 추가했다. 전체 Release 테스트는 Protocol 3,
  Core 11, Helper 61, LocalizationTool 6, App/WPF 16으로 총 97개가 통과했고 전체 Release
  빌드는 경고 0, 오류 0이다.
- 새 payload로 안전 제거·재설치를 실행해 manifest 233개가 누락 0, SHA-256 불일치 0임을
  확인했다. 라이브 게임 PID의 새 pipe에서 `testModeEnabled=true`와 capability 8개를
  확인했다. 메인 메뉴에서는 플레이어/인벤토리/왕국 저장소가 미로드이므로 두 자원 조회가
  각각 `FEATURE_UNAVAILABLE`을 정확히 반환했으며 Helper 오류 로그는 없었다.
- 병합 전 코드 리뷰에서 기력 싱글턴 교체, 자원 부분 적용, 잘못된 숫자 입력, 증감 입력값
  유지, 콤보 대비, 지원 범위 끝값 노출을 점검했다. 대상별 원본값 래치, 요청/실제 수량
  일치 검증, 문자열 입력 검증과 결과 분리, `0.1x`/`100x` 프리셋을 반영한 재검토에서
  새 Critical/Important 이슈 없이 병합 가능 판정을 받았다.
